...
async function main() {
  ...
  const token = await Token.deploy("web3Bridge", "VIII", 500);
  ...
  const USDT = await ethers.getContractFactory("CVIII");
  const usdt = await USDT.deploy("Tether", "USDT", 200);
  await usdt.deployed();
  ...
  const staker1Minting = await usdt
    .connect(holder1)._mint(100);
